...
// VERSION 3.0: Added configurable pickup threshold
// - Added menu item '4' to set the pot pickup threshold.
// - Moved pickup threshold into the EEPROM config struct.
...
//================================================================
const String FIRMWARE_VERSION = "3.0"; // MODIFIED: Version bump
...
struct JoystickConfig {
  ...
  bool isRightHanded;
  int kalmanLevel;
  int pickupThreshold; // NEW: Added to config struct
};

JoystickConfig config;
const int CONFIG_VERSION = 5; // MODIFIED: Incremented due to struct change
...
int yawValue = 0;
int thrustValue = 0;
int propellerValue = 128; 
bool lastPropellerMode = false;
bool potiControlEngaged = true;
// REMOVED: const int PICKUP_THRESHOLD = 20; // This is now in the config struct
...
void loop() {
  ...
  if (lastActivationButtonState == HIGH && currentActivationButtonState == LOW) {
    ...
    if (waitingForSecondClick && (currentTime - firstClickTime < doubleClickWindow)) {
  ...
  if (potiControlEngaged) {
    ...
    // MODIFIED: Use the value from the config struct
    if (propellerMode ? (abs(finalPotValue - propellerValue) < config.pickupThreshold) : (abs(finalPotValue - thrustValue) < config.pickupThreshold)) {
...
void loadConfiguration() {
  EEPROM.get(0, config);
  // MODIFIED: Check for new config version and set defaults
  if (config.version != CONFIG_VERSION) {
    config.version = CONFIG_VERSION;
    config.leanAngle = 40;
    config.isRightHanded = true;
    config.kalmanLevel = 1;
    config.pickupThreshold = 20; // NEW: Set default value
  }
...
void runSetupMenu() {
  ...
  while (true) {
    if (needsRedraw) {
      ...
      Serial.print("  4. Pickup Threshold: "); Serial.println(config.pickupThreshold); // NEW: Display new option
      Serial.println("\nOptions:");
    ...
    if (Serial.available() > 0) {
      ...
      if (choice == '1') {
        ...
        applyKalmanSettings(config.kalmanLevel);
        needsRedraw = true;
      } else if (choice == '4') { // NEW: Logic for new menu item
        Serial.println("\n> Enter new pickup threshold (5-50):");
        while (Serial.available() == 0) {}
        int newThreshold = Serial.parseInt();
        if (newThreshold >= 5 && newThreshold <= 50) {
          config.pickupThreshold = newThreshold;
          Serial.println("--> Pickup threshold updated.");
        } else {
